...
def compute_xi_for_bins(bins, randoms_bins, config):
    xi_list = []
    varxi_list = []
    s_list = []

    for i, (gxs, rxs) in enumerate(zip(bins, randoms_bins)):
...
def plot_bin_data_and_randoms(
    ...
    axes[0].scatter(
        rxs["ra"], rxs["dec"],
        s=1.5, color="k", alpha=0.15, label="Randoms"
    )
    axes[0].scatter(
        gxs["ra"], gxs["dec"],
...
def main():
    ...
    randoms_bins = build_randoms_for_bins(bins, random_data)

    print("Computing xi for each dist_fil bin")
    xi_list, varxi_list, s_list = compute_xi_for_bins(
        bins, randoms_bins, config
    )

    print("Plotting spatial + redshift distributions per bin")
    for i, (gxs, rxs, lab) in enumerate(zip(bins, randoms_bins, labels)):
