"""
- Sigma-based subsampling of galaxies instead of distance to filaments
"""
...
def split_by_sigma3(cat, column='sigma3'):
    values = cat[column].values

    if sigma3_bin_mode == "custom_intervals":
        bins = []
        labels = []
        for i, interval_list in enumerate(sigma3_bin_intervals):
            ...
            subset = cat.loc[mask_total].copy()
            bins.append(subset)
            labels.append(f"$\Sigma_3 \\in [{', '.join(label_parts)}]$ Mpc/h")
        return bins, labels

    elif sigma3_bin_mode == "percentile":
        percentiles = np.linspace(0, 100, nbins_sigma3 + 1)
        edges = np.percentile(values, percentiles)
    elif sigma3_bin_mode == "equal_width":
        vmin, vmax = values.min(), values.max()
        edges = np.linspace(vmin, vmax, nbins_sigma3 + 1)
    elif sigma3_bin_mode == "fixed":
        edges = np.array(sigma3_bin_edges)
    elif sigma3_bin_mode == "percentile_intervals":
        bins = []
        labels = []
        for (lo_pct, hi_pct) in sigma3_bin_percentile_intervals:
            ...
            bins.append(subset)
            labels.append(f"$\Sigma_3 \\in [{lo_val:.1f}, {hi_val:.1f}]$ Mpc/h")
        return bins, labels
    else:
        raise ValueError(f"Unknown sigma3_bin_mode: {sigma3_bin_mode}")

    # For percentile, equal_width, fixed
    bins = []
    labels = []
    for i in range(len(edges) - 1):
        ...
        labels.append(f"${lo:.1f} < \Sigma_3 \\leq {hi:.1f}$")
    return bins, labels
...
def main():
    ...
    plot_monopoles_combined(monopoles_list, labels_list,
                            ...
                            filename='xi0_combined_sigma.png')
