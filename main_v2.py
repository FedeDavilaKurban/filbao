...
def split_filaments_and_randoms(cat_z_mag: pd.DataFrame, random_data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    nonfilgxs = cat_z_mag[cat_z_mag["dist_fil"] > p66]
...
def plot_xi(xi, varxi, s, xi_fil, varxi_fil, s_fil, xi_nonfil=None, varxi_nonfil=None, s_nonfil=None, plotname=None) -> None:
    sig = np.sqrt(varxi)
