...
# ---- Sample ----------
sample = 'nyu'
test_dilute = .6                     # fraction of galaxies to keep (1.0 = full)
sigma = 3.0
h = 0.6774
zmin, zmax = 0.07, 0.2
mag_max = -21.2
...
def xi_sigmapi_package(ra_data, dec_data, chi_data,
    ...
    """
    Compute ξ(σ, π) using Corrfunc and save a plot.
    """
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Rebining in π
    npi_new = pimax // pi_rebin
    H_dd_rebinned = H_dd.reshape(nbins, npi_new, pi_rebin).sum(axis=2)
    H_rr_rebinned = H_rr.reshape(nbins, npi_new, pi_rebin).sum(axis=2)
    H_dr_rebinned = H_dr.reshape(nbins, npi_new, pi_rebin).sum(axis=2)
    ...
    im = ax.imshow(xi.T, origin='lower', extent=extent,
...
def main():
    ...
    for i, (gxs, rxs) in enumerate(zip(bins, randoms_bins_list)):
        print(f"Computing ξ(σ, π) for bin {i}...")
        xi_sigmapi_package(
